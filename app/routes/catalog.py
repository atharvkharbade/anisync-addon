...
async def handle_catalog(user_id: str, catalog_type: str, catalog_id: str, extras: str = ""):
    ...
    if catalog_id in dubbed_cat_ids:
        enable_dubbed = user.get("enable_dubbed_catalogs", False) if user else False
        user_catalogs = user.get("catalogs") if user else None
        if not enable_dubbed and (user_catalogs is None or catalog_id not in user_catalogs):
            return await respond_with({"metas": []})
        ...
        user_dub_lang = user.get("dubbed_language", "english") if user else "english"

        base_cat_map = {
            "anisync_dubbed_trending": "anisync_trending",
            "anisync_dubbed_seasonal": "anisync_seasonal",
            "anisync_dubbed_popular": "anisync_most_popular",
            "anisync_dubbed_movies": "anisync_spotlight",
        }
        base_cat_id = base_cat_map.get(catalog_id, "anisync_trending")

        genre = filters.get("genre")
        cache_key = f"{base_cat_id}:{genre}" if (genre and base_cat_id in ["anisync_seasonal", "anisync_spotlight"]) else base_cat_id

        cached = None
        base_cached = None
        try:
            base_cached = discovery_col.find_one({"catalog_id": base_cat_id})
            if genre and base_cat_id in ["anisync_seasonal", "anisync_spotlight"]:
                cached = discovery_col.find_one({"catalog_id": cache_key})
                if (not cached or not cached.get("metas")) and base_cached:
                    cached = base_cached
            else:
            ...
            logging.error("Failed to query discovery_catalogs_cache for dubbed catalog: %s", e)

        raw_metas = []
        if cached and cached.get("metas") and cached.get("expires_at", now) > now:
            raw_metas = cached["metas"]
        elif base_cached and base_cached.get("metas"):
            raw_metas = base_cached["metas"]
        else:
            try:
                all_metas = await update_discovery_catalogs_cache()
                raw_metas = all_metas.get(cache_key) or all_metas.get(base_cat_id, [])
            except Exception as e:
                logging.error("Failed to update discovery catalogs for dubbed catalog: %s", e)
                raw_metas = []

        # Filter by genre if genre is an anime genre (e.g. Action, Comedy, Romance, etc.)
        if genre and genre not in [
            "Current Season", "Next Season", "Upcoming", "Winter", "Spring", "Summer", "Fall",
            "Feature Films", "New Movies", "OVAs & Specials", "Classic Masterpieces"
        ]:
            g_lower = genre.strip().lower()
            raw_metas = [m for m in raw_metas if any(g_lower == str(g).strip().lower() for g in m.get("genres", []))]

        # Filter to only dubbed anime in the user's chosen language
        metas = await filter_dubbed(raw_metas, language=user_dub_lang)
    ...
    # --- Combined Watchlists ---
        ...
        try:
            ...
            def compute_comb_flags(item):
                ...
                al_status = al_media.get("status") if isinstance(al_media, dict) else ""
            ...
            for item in paged_items:
                try:
                    ...
                    trackers = []
                    if item.get("mal_item") and user.get("mal_enabled"):
                        trackers.append("mal")
                    if item.get("anilist_item") and user.get("anilist_enabled"):
                        trackers.append("anilist")
                    if item.get("simkl_item") and user.get("simkl_enabled", True) and user.get("simkl_access_token"):
                    ...
                    if provider_pref == "custom":
                        provider_pref = (user.get("meta_poster_provider") or user.get("meta_artwork_provider") or "anilist").lower()
                    ...
                    def extract_mal():
                        ...
                        if item.get("mal_item"):
                            node = (item["mal_item"].get("node") or {}) if isinstance(item["mal_item"], dict) else {}
                            progress = max(progress, (node.get("my_list_status") or {}).get("num_episodes_watched", 0) or 0)
                            if total_eps == "?":
                    ...
                    if is_new_ep and enable_new_ep_badge and poster:
                        ...
                        if item.get("mal_item"):
                            ...
                            short_trackers.append("m")
                        if item.get("anilist_item"):
                            active_trackers.append("anilist")
                            short_trackers.append("a")
                        if item.get("simkl_item"):
                            active_trackers.append("simkl")
                        ...
                        poster = f"{Config.PROTOCOL}://{Config.REDIRECT_URL}/{user_id}/poster/{m_id_for_url}_{short_tracker_str}_22.jpg?url={encoded_url}&badge=new&tracker={trackers_param}&style={badge_style}&v=hd_poster_v1"
